...
function render(){
  ...
  for(var row=0;row<mc;row++){for(var col=0;col<mc;col++){
    ...
    if(isEP(row,col)){
      if(ES==='rounded'){rr(ctx,x,y,s,s,s*.4);ctx.fill();}
      else if(ES==='extra-rounded'){rr(ctx,x,y,s,s,s*.5);ctx.fill();}
      else if(ES==='dots'){ctx.beginPath();ctx.arc(x+cell/2,y+cell/2,cell/2*.9,0,Math.PI*2);ctx.fill();}
  ...
  if(LI){var lw=QSZ*(LSZ/100),lx=qX+(QSZ-lw)/2,ly=qY+(QSZ-lw)/2,lr=lw*.18;ctx.save();if(LBG){ctx.shadowColor='rgba(0,0,0,.12)';ctx.shadowBlur=7;ctx.fillStyle='#fff';rr(ctx,lx-5,ly-5,lw+10,lw+10,lr+4);ctx.fill();ctx.shadowBlur=0;}rr(ctx,lx,ly,lw,lw,lr);ctx.clip();ctx.drawImage(LI,lx,ly,lw,lw);ctx.restore();}
